...
app.post('/', [express_validator_1.body('id', "id can't be empty").toInt(),
    express_validator_1.body('faqId', "faqId can't be empty").toInt(),
    express_validator_1.body('Question', "Question can't be empty").isString().isLength({ min: 1 }),
    express_validator_1.body('Answer', "Answer can't be empty").isString().isLength({ min: 1 }),
    ...
    const newUpgrade = yield controller_1.createNewFaqQnA(id, faqId, Question, Answer); //one more field of frameId
...
app.get('/init', helper_1.expressQAsync((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const allFaqs = yield controller_1.initNewFaqQnA();
    console.log(allFaqs);
    const response = helper_1.createResponse("OK", allFaqs, undefined);
    res.send(response);
})));
app.delete('/', [express_validator_1.body('id', "id can't be empty").toInt(),
    express_validator_1.body('faqId', "faqId can't be empty").toInt(),
    helper_1.validate], helper_1.expressQAsync((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    yield controller_1.deleteUpgrade(req.body.id, req.body.faqId);
